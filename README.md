...
### 制作顶部滑动条的坑们：
 ...
 4. 我们在初始化 滑动条 的时候，导入的 mui.js ，但是，控制台报错： `Uncaught TypeError: 'caller', 'callee', and 'arguments' properties may not be accessed on strict mode`
  ...
  + 最终，我们选择了 plan B  移除严格模式： 使用这个插件npm install babel-plugin-transform-remove-strict-mode, 将会在package.json中加入该依赖，同时在.babelrc中加入移除严格模式的配置，"transform-remove-strict-mode"
